...
export class GameScene extends Phaser.Scene {
  ...
  private activateChunk(chunk: ChunkCoord): void {
    ...
    // Objects (buildings, the well) don't have real art yet, so they still
    // go through chunkScratch as flat-color Graphics diamonds; terrain is
    // stamped straight onto the RenderTexture from the blob tileset (see
    // tools/tileset-gen/src/tileset_gen/blob.py) generated by
    // tools/tileset-gen. The two never cover the same tile, so draw order
    // between them doesn't matter — chunkScratch is just blitted on top
    // once, below.
    //
    // Terrain rendering, per tile:
    //  1. A water safety-base tile, unconditionally. Fully covered by
    //     everything below in every real case; kept as a defensive
    //     fallback so a bug in the wedge logic reads as "a patch of
    //     water" during development, not a hole straight to the render
    //     texture's own transparency (which showed as solid black — this
    //     is how an earlier version of this scheme's gap actually got
    //     caught).
    //  2. Per TERRAIN_PRIORITY entry (lowest first): if this tile's own
    //     terrain matches, its own cut blob (see ownCutMaskFor — cut only
    //     toward HIGHER-priority neighbours; a higher-priority terrain is
    //     therefore never cut at all). Otherwise, if this pass's terrain
    //     outranks the tile's own terrain and is one of its edge/corner
    //     neighbours, a wedge of that terrain (edgeWedgeKey/
    //     cornerWedgeKey — the exact alpha inverse of the cut, see
    //     tileset.ts) encroaching from that direction — this is what a
    //     cut in step 2 actually reveals: not "whatever's underneath,"
    //     but a wedge shaped to fit it exactly, drawn at the SAME
    //     position.
    //
    ...
    for (let row = rowStart; row < rowEnd; row++) {
      for (let col = colStart; col < colEnd; col++) {
        if (this.grid.getObjectAt(col, row)) continue; // drawn separately, below
        const p = gridToScreen(col, row);
        const x = p.x - minX - TILE_WIDTH / 2;
        const y = p.y - minY - TILE_HEIGHT / 2;
        // Unlike stamp(), batchDrawFrame draws from the top-left corner,
        // not centered on (x, y) — offset by half the tile so it lines up
        // with everything else that treats (x, y) as the diamond's
        // centre (gridToScreen, drawDiamond).
        const safetyKey = terrainTileKey(TerrainType.Water, 0, col, row);
        texture.batchDrawFrame(safetyKey, undefined, x, y);
      }
    }
    for (const terrain of TERRAIN_PRIORITY) {
      ...
      for (let row = rowStart; row < rowEnd; row++) {
        for (let col = colStart; col < colEnd; col++) {
          if (this.grid.getObjectAt(col, row)) continue;
          const trueTerrain = this.grid.getTerrain(col, row);
          const p = gridToScreen(col, row);
          const x = p.x - minX - TILE_WIDTH / 2;
          const y = p.y - minY - TILE_HEIGHT / 2;

          if (trueTerrain === terrain) {
            const mask = ownCutMaskFor(this.grid, col, row, terrain);
            const key = terrainTileKey(terrain, mask, col, row);
            texture.batchDrawFrame(key, undefined, x, y);
          } else if (rank > terrainPriorityRank(trueTerrain)) {
            // ownCutMaskFor's reduction already worked out exactly which
            // of this tile's edge/corner bits `terrain` is responsible
            // for (a corner only survives reduction when both its
            // flanking edges are NOT cut) — reuse it instead of
            // rederiving the same eligibility check here.
            const ownMask = ownCutMaskFor(this.grid, col, row, trueTerrain);
            for (const { dCol, dRow, bit } of EDGE_OFFSETS) {
              if (!(ownMask & bit)) continue;
              const nCol = col + dCol;
              const nRow = row + dRow;
              if (this.grid.getTerrain(nCol, nRow) !== terrain) continue;
              const wedgeKey = edgeWedgeKey(terrain, bit);
              texture.batchDrawFrame(wedgeKey, undefined, x, y);
            }
            for (const { dCol, dRow, bit } of CORNER_OFFSETS) {
              ...
              if (this.grid.getTerrain(nCol, nRow) !== terrain) continue;
              const wedgeKey = cornerWedgeKey(terrain, bit);
              texture.batchDrawFrame(wedgeKey, undefined, x, y);
            }
          }
        }
