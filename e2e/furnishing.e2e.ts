 ...
 *
 * **The second half of `house.e2e.ts`**, and split from it for the
 * browser's sake rather than the reader's: one file gets one browser, every
 * reload re-boots the whole game, and a browser asked for enough pages in a
 * row stops answering. Ten scenarios in one file took seven minutes and hung
 * on the last; the same ten pass in two halves.
 *
 * The helpers below are that file's, copied rather than shared. Scenario
 * files here are self-contained on purpose — what a test reaches for should
 * be visible in the file doing the reaching — and fifty lines of walking
 * home is a cheaper duplication than a module nobody can read a scenario
 * without.
 */
